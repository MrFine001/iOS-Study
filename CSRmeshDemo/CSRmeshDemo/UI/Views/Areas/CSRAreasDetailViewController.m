...
- (IBAction)saveAreaConfiguration:(id)sender
{
    if(_areaEntity) {
        ...
        if ([_areaEntity.favourite boolValue])
            [[CSRWatchManager sharedInstance]updateFavourite:_areaEntity];
    } else {
...
{
    if ([_areaEntity.favourite boolValue]) {
        [[CSRWatchManager sharedInstance] deleteFavourite:_areaEntity.areaID];
    }
