...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    _titleLabel.text = _gateway.name;
    
    _gatewayIcon.image = [CSRmeshStyleKit imageOfGateway];
...
- (void)refreshTableDetails:(id)sender
{
    ...
    gatewayDetails = @[@{@"detailName":@"UUID", @"detailValue":_gateway.uuid},
                       @{@"detailName":@"IP", @"detailValue":_gateway.host},
                       @{@"detailName":@"Device ID", @"detailValue":[NSString stringWithFormat:@"%@ (0x%04x)", _gateway.deviceId, [_gateway.deviceId unsignedShortValue]]}];
...
- (IBAction)deleteButtonTapped:(id)sender
{
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"Do you want to remove %@?", _gateway.name]
                                                                             message:nil
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Yes"
                                                     ...
                                                     handler:^(UIAlertAction *action) {
                                                         ...
                                                         switch ([_gateway.state intValue]) {
...
- (void)checkGatewayState
{
    ...
    switch ((CSRGatewayState)[_gateway.state intValue]) {
