...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([settingsItemDict[@"name"] isEqualToString:@"Automatic switch"]) {
        ...
        cell = [tableView dequeueReusableCellWithIdentifier:CSRActionCellIdentifier];
        if (!cell) {
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CSRActionCellIdentifier];
        }
        
        ((CSRActionTableViewCell *)cell).titleLabel.text = settingsItemDict[@"name"];
        
        if (settingsItemDict[@"detail"]) {
            ((CSRActionTableViewCell *)cell).detailLabel.text = settingsItemDict[@"detail"];
        }
...
- (id)selectedBearerOption:(id)option
{
    ...
    switch ([((NSNumber *)option) integerValue]) {
        ...
        case 0:
        {
            ...
            if (((CSRBluetoothLE *)[CSRBluetoothLE sharedInstance]).cbCentralManagerState == CBCentralManagerStatePoweredOn) {
                ...
                bearerOptionName = @"Bluetooth";
                [[CSRAppStateManager sharedInstance] switchConnectionForSelectedBearerType:(CSRSelectedBearerType)[((NSNumber *)option) integerValue]];
