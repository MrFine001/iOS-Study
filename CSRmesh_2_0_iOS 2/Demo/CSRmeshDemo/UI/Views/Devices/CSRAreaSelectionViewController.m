...
- (void)viewDidLoad
{
    ...
    //_saveButton.image = [CSRmeshStyleKit imageOfBack_arrow];
    [_saveButton setAction:(@selector(save:))];
    _saveButton.action = @selector(save:);
    _saveButton.target = self;
    
    _searchButton.image = [CSRmeshStyleKit imageOfSearch];
    [_searchButton setAction:(@selector(showSearch:))];
    _searchButton.action = @selector(showSearch:);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    _areasArray = [[[[CSRDatabaseManager sharedInstance] fetchObjectsWithEntityName:@"CSRAreaEntity" withPredicate:nil] allObjects] mutableCopy];
...
- (IBAction)save:(id)sender
{
    ...
    for (NSNumber *desired in allGroupsData) {
                                             ...
                                             groupIndex:@(groupIndex)
                                               instance:@(0)
                                                ...
                                                success:^(NSNumber *deviceId,
                                                    ...
                                                    *group = [desired unsignedShortValue];
                                                    
                                                    
                                                    // Add or remove area to device
                                                    if ([desired unsignedShortValue] == 0) {
                                                        desired = @(groupToBeReplaced);
                                                    }
                                                    
                                                    CSRAreaEntity *areaEntity = [[[[CSRDatabaseManager sharedInstance] fetchObjectsWithEntityName:@"CSRAreaEntity" withPredicate:@"areaID == %@", desired] allObjects] firstObject];
